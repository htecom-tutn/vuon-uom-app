...
class ConvertDataServices implements ConvertDataInterface
{
    ...
    public function cvDataFromTrongTrePro(){
        try {
            ...
            Log::info('Lỗi convert dữ liệu báo cáo tồn kho theo vị trí: ' . Carbon::now() . '-' . $th->getMessage());    
        }
    }
    private function DataTTPro(){
        $totalRecord = $this->tUsersRepository->count();
        $limit = 1000;
    ...
    public function convertDataFromTrongTrePro(){
        try {
            $data = $this->getDataTT();                    
            // Data mới của bảng vườn ươm    
            $status = config('app.data.active'); //1        
